...
|===
|*Consideration*|*Alternative one*|*Alternative two*
|Method of tracking multiple projects within project list and multiple tasks within each project
a|Each project will have its own *_<Project File>.json_* to keep track of tasks.
...
All projects within project list and all tasks of all projects contained in *_project.json_* file.

** Pros: Will not clutter data folder.

** Cons: Entire project list has to be written to file every time project list is modified.

This option was chosen as it facilitates importing and exporting projects to one file only.
|===
// end::importexport[]
...
image::compare1.png[width="900", title="Example of VersionedProject when `edit` on Task B"]

Given above (Figure 8.) is an example usage scenario and how the Compare Task mechanism behaves.
...
image::compare2.png[width="900", title="Process of how `compare` command will flow"]

Step 4. `VersionedProject#compareTask()` will iterate through all the the tasks within each project state. The `taskId`
will be used to determine if the task encountered during the iteration is the same as the chosen task (Figure 9, A).
...
Step 5. If the task encountered is the same as the chosen task, the 2 task will be compared against their name, description & deadline (Figure 9, B).

image::compare 3.png[width="800", title="A successful `compare` where name is different"]

Step 6. If there is a difference in any of the fields in the comparison (Figure 10), the difference will be showed to the user.
Else Step 4. will continue to run to look for another version of the chosen task to compare (Figure 9, C).
...
image::compareseq.png[width="900", title="Sequence Diagram for `compare` Command"]

Given above (Figure 11) is the sequence diagram when `compare` is called.
// end::compare[]
...
image::ViewTaskHistorySeq1.png[width="900", title="Behaviour of userTaskHistoryTaskId during `add` commands"]

Given above (Figure 12) is an example usage scenario of how the Task History mechanism behaves.

Note the behaviour of useTaskHistoryTaskId when all other commands (not `edit`) is called (Figure 12). `CommandHistory#addHistoryTaskId()` will not be called thus `userInputHistoryTaskId` will not be updated.
However `userInputHistory` will still be updated as usual.

image::ViewTaskHistorySeq2.png[width="900", title="Behaviour of userTaskHistoryTaskId during `edit` commands"]

Note the behaviour when a `edit` command  is called (Figure 13). `CommandHistory#addHistoryTaskId()` will be called thus `userInputHistoryTaskId` will be updated with the taskId of the task where the
...
==== Sequence Diagram (`taskhistory`)

image::taskhistoryseq.png[width="900", title="Sequence Diagram for `taskhistory` Command"]
