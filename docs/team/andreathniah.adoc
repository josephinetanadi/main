...
**NoteNote** is the go-to task management tool built for computing students to keep track of all your task right at your fingertips. With a minimalistic command line interface (CLI) which users would be more than familiar with as computing students, NoteNote is an efficient tool that sorts through tasks quicky and efficiently. NoteNote also allows users to analyze their productivity, categorize their tasks, share tasks with their friends and many more.

This application was developed by a team of four for CS2113T, an module from the National University of Singapore over 13 weeks.

.Application Layout
image::applayout.png[width="800"]
