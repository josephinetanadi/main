...
image::CompletedOutcome.png[width="600", title="`completed` tag (in pink) is displayed for a completed task"]
...
image::AnalyseOutcome.png[width="790", title="Statistics produced as a result of `analyse`"]
...
image::CmdAlias.png[width="600", title="`completed` command (left) versus its shorthand alias (right)"]
